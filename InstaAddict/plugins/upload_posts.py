...
class UploadPostsPlugin(Plugin):
            ...
            for file in files:
                ...
                try:
                    ...
                    if mtime > limit_threshold:
                        logger.info(
                            f"Upload rate limit active. Waiting {RATE_LIMIT_HOURS}h+ to upload next queued post."
                        )
                        return True
                except OSError as e:
                    logger.debug(
    ...
    def run(
        ...
        username: str = (
            config_path.split("/")[-2]
            if "/" in config_path
            else config_path.split("\\")[-2]
        )

        pending_dir: str = os.path.join(
            "accounts", username, "content_queue", "pending"
        )
        published_dir: str = os.path.join(
            "accounts", username, "content_queue", "published"
        )
        ...
        for media_file in media_files:
            ...
            if not caption:
                logger.info(
                    f"No caption provided for {media_file}. Engaging AI Autopilot Vision-Captioner..."
                )
                import yaml
    ...
    def _upload_to_ig(self, device: Any, media_path: str, caption: str) -> bool:
        ...
        create_btn = device.deviceV2(
            descriptionMatches="(?i)New post.*|(?i)Camera.*|(?i)Create"
        )
        if not create_btn.exists(timeout=5):
            tab_bar = device.deviceV2(resourceIdMatches=".*tab_bar.*").child(
        ...
        if share_btn.exists(timeout=5):
            ...
            in_feed: bool = device.deviceV2(descriptionMatches="(?i).*Home.*").exists(
                timeout=5
            )
            if in_feed:
                return True
